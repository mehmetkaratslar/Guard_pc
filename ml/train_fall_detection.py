...
def train_fall_detection_model(data_dir, output_model_path, batch_size=8, num_epochs=30, learning_rate=0.001):
    ...
    # Eğitimden önce çalıştır:
    corrupt_images = find_corrupt_images(normal_images + fall_images)

    if corrupt_images:
        print("Bozuk görüntüler tespit edildi:")
        for img in corrupt_images:
            print(f" - {img}")
        raise RuntimeError("Bozuk görüntüler bulundu, lütfen düzeltin.")
